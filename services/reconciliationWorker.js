  ...
  async (job) => {
    ...
    for (const transaction of staleTransactions) {
      try {
        ...
        const gatewayStatus = await gateway.verifyTransfer(
        ...
        if (gatewayStatus === "SUCCESSFUL") {
          ...
          console.log(
            `[Reconciliation] Transaction ${transaction._id} — still ${gatewayStatus} on ${gatewayName}, will check next cycle`,
          );
        }
      } catch (error) {
    ...
    console.log(
      `[Reconciliation] Job completed at ${new Date().toISOString()}`,
    );
  },
  {
    connection,
